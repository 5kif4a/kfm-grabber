...
from settings import SECRET_KEY, SENTRY_DSN
...
def put(table):
    try:
        ...
        for k in data.keys():
            if data[k] != obj.__dict__[k]:
                session.query(model).filter_by(index=idx).update(data)
                last_idx = len(session.query(History).all())
                hist = History(index=last_idx, table=table, obj_id=int(idx), note='Data changed')
                session.add(hist)
                break
        res = make_response(jsonify({"message": "OK"}), 200)
...
@celery.task(name="periodic_task")  # async task, work every hour
def periodic_task():
    try:
        updater.run()
    except Exception as e:
